...
// OrderDetailPage shares styles with OrdersPage (all .od-* classes live there)
              ...
              {!isCancelled && (
                <div className="od-progress">
                  {STEPS.map((s, i) => (
                    <div
                      key={s}
                      className={`od-step ${i + 1 <= step ? "od-step--done" : ""} ${i + 1 === step ? "od-step--active" : ""}`}
                    >
                      <div className="od-step__dot">{i + 1 <= step ? "✓" : i + 1}</div>
                      <p className="od-step__label">{s}</p>
                      {i < STEPS.length - 1 && (
                        <div className={`od-step__line ${i + 1 < step ? "od-step__line--done" : ""}`} />
                      )}
                    </div>
                  ))}
                </div>
