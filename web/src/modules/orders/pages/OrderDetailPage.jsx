...
import { fetchOrder, confirmPayment } from "../api/ordersApi.js";
...
export default function OrderDetailPage() {
  ...
  return (
    <>
      <Navbar />
      <div className="od" style={{ marginTop: "var(--navbar-h)" }}>
        <div className="od-container">
          ...
          <div className="od-layout">
            <div>
              <div className="od-status-card">
                <div>
                  ...
                  <p className="od-date">
                    Placed on {new Date(order.createdAt).toLocaleDateString("en-PH", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                  </p>
                </div>
              ...
              {isPending && payInfo && (
                <div className="od-pay-instructions">
                  ...
                  <button className="od-confirm-pay-btn" onClick={handlePay} disabled={paying}>
                    {paying ? "Confirming…" : "Confirm Payment"}
                  </button>
                  <p className="od-pay-note">* This is a simulated payment for demo purposes.</p>
                </div>
              )}
                  ...
                  <div key={item.productId} className="od-item">
                    <div className="od-item__img">
                      {item.productImageUrl
                        ? <img src={item.productImageUrl} alt={item.productName} onError={(e) => { e.currentTarget.style.display = "none"; }} />
                        : <span>🐠</span>}
                    </div>
                    <div className="od-item__info">
                      <button className="od-item__name" onClick={() => navigate(`/products/${item.productId}`)}>
                        {item.productName}
                      </button>
                    ...
                    <div className="od-item__right">
                      ...
                      <p className="od-item__subtotal">₱{Number(item.subtotal).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <aside className="od-summary">
              ...
              <div className="od-detail-row"><span>Order Reference</span><strong>{order.orderRef}</strong></div>
              <div className="od-detail-row"><span>Payment Method</span><strong>{order.paymentMethod.replace("_", " ")}</strong></div>
              <div className="od-detail-row"><span>Status</span><strong style={{ color: cfg.color }}>{cfg.label}</strong></div>
              <div className="od-divider" />
              <div className="od-detail-row"><span>Subtotal</span><span>₱{Number(order.totalAmount).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</span></div>
              <div className="od-detail-row"><span>Shipping</span><span style={{ color: "#3D7A56", fontWeight: 600 }}>Free</span></div>
              <div className="od-divider" />
